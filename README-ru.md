...
Альтернативно, добавьте в зависимости вашего `Package.swift`:
...
Letopis предоставляет встроенную поддержку маскирования конфиденциальной информации в payload логов для предотвращения случайного раскрытия паролей, токенов, API ключей и других секретных данных.

#### Стратегии маскирования

Доступны четыре стратегии маскирования через `SensitiveDataStrategy`:

- **`.full`** – Заменяет всё значение на `***`  
  Пример: `"secret123"` → `"***"`

- **`.partial`** – Сохраняет первый и последний символ, маскирует остальное  
  Пример: `"secret123"` → `"s***3"`

- **`.email`** – Маскирует пользовательскую часть email  
  Пример: `"user@example.com"` → `"u***@example.com"`

- **`.last4`** – Показывает только последние 4 символа (полезно для номеров карт)  
  Пример: `"1234567890123456"` → `"***3456"`

#### Глобальные чувствительные ключи

Настройте чувствительные ключи глобально при создании логгера:
...
// Добавьте ключи динамически
logger.addSensitiveKeys(["credit_card", "secret"])

// Удалите ключи при необходимости
logger.removeSensitiveKeys(["api_key"])
```

Глобальные ключи используют стратегию `.partial` по умолчанию при включенном маскировании.

#### Маскирование для конкретного лога

Включите маскирование для конкретных логов через fluent API:

```swift
// Используем глобальные чувствительные ключи
logger.log()
...
// Маскируем конкретные ключи с пользовательскими стратегиями
logger.log()
...
// Маскируем несколько ключей одной стратегией
logger.log()
...
- Маскирование применяется только при вызове `.sensitive()` в цепочке лога
- Пользовательские стратегии ключей имеют приоритет над глобальными чувствительными ключами
- Маскирование происходит до того, как событие достигнет интерцепторов
